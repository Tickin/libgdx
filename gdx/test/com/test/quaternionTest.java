...
public class quaternionTest {
	...
	public void testToMatrix(){
		...
		expected[0]=1-2*(y*y+z*z);
		expected[1]=2*(x*y+z*w);
		expected[2]=2*(x*z-y*w);
		expected[3]=0;
		expected[4]=2*(x*y-z*w);
		expected[5]=1-2*(x*x+z*z);
		expected[6]=2*(y*z+x*w);
		expected[7]=0;
		expected[8]=2*(x*z+y*w);
		expected[9]=2 * (y*z - x*w);
		expected[10]=1-2*(x*x+y*y);
		expected[11]=0;
		expected[12]=0;
		expected[13]=0;
		expected[14]=0;
